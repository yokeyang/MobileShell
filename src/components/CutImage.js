...
/*					$('.resize-point').off('mousedown');
	...
	addResizeBtn: function(){
		...
		$('#canvas-box').append($seResize);
		$('#canvas-box').append($swResize);
		$('#canvas-box').append($neResize);
		$('#canvas-box').append($nwResize);
	...
	save: function(){
		...
		var self = this;
		var height = self._img_show.crop_height*10;
		var width = self._img_show.crop_width*10;
		var $result = $("<canvas class=result width='"+ width +"' height='"+ height +"'></canvas>");
		var $viewresult = $("<canvas class=viewresult width='"+ width/10 +"' height='"+ height/10 +"'></canvas>");
		try {			
			$('.viewresult').remove()
		} catch (error) {
			console.log(error)
		}
		$('#canvas-box').append($viewresult);
		try{
			$result[0].getContext('2d').drawImage(self._img,
				self._save.left, self._save.top, self._save.width, self._save.height,
				0, 0, width, height
			);
			$viewresult[0].getContext('2d').drawImage(self._img,
				self._save.left, self._save.top, self._save.width, self._save.height,
